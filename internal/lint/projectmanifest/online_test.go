...
func TestCheckArgumentsO2EnumViolationMessageIsReadable(t *testing.T) {
	...
	// `jsonschema: '' does not validate with file:///.../arguments/foo#/enum: ...`,
	...
	idx, _ := buildArgIndex(mods)
	res := &LoadResult{Manifest: &configuration.ServiceManifest{
		Arguments: map[string]interface{}{"foo": "three"}, // not in the enum
	}}
	findings := checkArguments(res, idx)
	assert.Equal(t, 1, len(findings))
	msg := findings[0].Message
	assert.Assert(t, contains(msg, `"three"`), msg)        // the actual offending value
	assert.Assert(t, contains(msg, "github.com/x/a"), msg) // the declaring module
